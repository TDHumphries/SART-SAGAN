...
def deconv(x, channels, kernel=4, stride=2, padding='SAME', use_bias=True, sn=False, scope='deconv_0'):
    with tf.variable_scope(scope):
        ...
        if sn :
            ...
            x = tf.layers.conv2d_transpose(inputs = x, filters = channels, kernel_size = kernel, strides=(stride, stride), padding=padding, data_format='channels_last', activation=None, use_bias=True, name = None)
